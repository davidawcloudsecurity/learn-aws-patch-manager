...
# ============================================================
# User Data Scripts (inline)
# ============================================================

locals {
  userdata_inproc = <<-INPROC
<powershell>
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .card { background: #fff; border-radius: 16px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); padding: 40px; max-width: 450px; width: 100%; }
    ...
    .login-form input[type="text"] { padding: 12px 20px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 1em; width: 80%; margin-bottom: 12px; }
    .login-form button { background: linear-gradient(135deg, #3b82f6, #1d4ed8); color: #fff; border: none; padding: 12px 28px; border-radius: 8px; font-size: 0.95em; cursor: pointer; font-weight: 600; width: 80%; }
    .login-form button:hover { opacity: 0.9; }
    .alert { border-radius: 10px; padding: 14px 18px; margin: 12px 0; font-size: 0.85em; background: #dbeafe; border: 1px solid #93c5fd; color: #1e40af; }
    .footer { text-align: center; margin-top: 20px; padding-top: 16px; border-top: 1px solid #e2e8f0; }
    .footer p { color: #94a3b8; font-size: 0.75em; }
    .badge { display: inline-block; font-size: 0.7em; padding: 3px 10px; border-radius: 20px; font-weight: 600; background: #fef3c7; color: #92400e; }
  </style>
</head>
<body>
  <div class="card">
    <div class="header">
      ...
      <p>Login to see how sessions behave during Instance Refresh</p>
    </div>
    <div class="alert">
      <strong>Demo:</strong> Login, then trigger an ASG Instance Refresh.
      Your session (InProc) will be lost when this instance is replaced.
    </div>
    ...
    <div class="footer">
      <p>Session Mode: <span class="badge">InProc (Server Memory)</span></p>
    </div>
  </div>
</body>
</html>
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .btn-red { background: linear-gradient(135deg, #ef4444, #b91c1c); color: #fff; border: none; padding: 12px 28px; border-radius: 8px; font-size: 0.95em; cursor: pointer; font-weight: 600; }
    .footer { text-align: center; margin-top: 20px; padding-top: 16px; border-top: 1px solid #e2e8f0; }
...
<body>
  ...
  <div class="card">
    ...
    <div class="section-title">Session State (InProc)</div>
    <div class="info-grid">
      <div class="info-item">
    ...
    <% } else { %>
      <div class="alert alert-blue">
        <strong>Sticky:</strong> Still on the same host. Trigger Instance Refresh to test.
      </div>
    <% } %>
    ...
    <div class="footer">
      <p>
        ...
        1. Sysprep gives unique hostname per instance<br/>
        2. InProc sessions die on instance termination<br/>
        3. Sticky sessions only work while the server lives<br/>
        4. Fix: Redis/DynamoDB/SQL session state or JWT
      </p>
      <p style="margin-top:8px;">Session Mode: <span class="badge badge-yellow">InProc</span></p>
    </div>
  </div>
</body>
</html>
...
# Set login.aspx as default document and remove IIS defaults
...
Remove-Item -Path "C:\inetpub\wwwroot\default.aspx" -ErrorAction SilentlyContinue

# Configure IIS default document to login.aspx
Set-WebConfigurationProperty -Filter "//defaultDocument/files" -PSPath "IIS:\Sites\Default Web Site" -Name "." -Value @{value="login.aspx"}
</powershell>
INPROC

  userdata_cookie_auth = <<-COOKIE
Install-WindowsFeature -Name Web-Server -IncludeManagementTools
Install-WindowsFeature -Name Web-Asp-Net45

# Fetch JWT signing key from Parameter Store
$signingKey = (Get-SSMParameter -Name "/${var.project_tag}/jwt-signing-key" -WithDecryption $true).Value
...
resource "aws_launch_template" "windows" {
  ...
  user_data = base64encode(var.use_cookie_auth ? local.userdata_cookie_auth : local.userdata_inproc)
