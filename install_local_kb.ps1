...
function Find-MatchingCAB {
    param(
        ...
        [string]$KBNumber,
        [string]$SearchPath = "C:\Windows\SoftwareDistribution\Download"
    )

    Write-Log "Searching for KB$KBNumber related CAB files in $SearchPath" -Level "Info"

    try {
        # Get all CAB files in the directory
        $cabFiles = Get-ChildItem -Path $SearchPath -Filter "*.cab" -Recurse -ErrorAction Stop
        ...
        $matches = $cabFiles | Where-Object {
            ...
            # Check file metadata
            (Get-ItemProperty $_.FullName).VersionInfo.FileDescription -match "KB$KBNumber" -or
            
            # Check package info using DISM
            (& dism /online /get-packages | Select-String -Pattern "KB$KBNumber") -and
            (& dism /Get-PackageInfo /PackagePath:$($_.FullName) -ErrorAction SilentlyContinue)
        }

        if ($matches) {
            Write-Log "Found potential matching CAB files:" -Level "Success"
            foreach ($match in $matches) {
                Write-Log "- $($match.FullName)" -Level "Info"
            }
            return $matches
        } else {
            Write-Log "No matching CAB files found for KB$KBNumber" -Level "Warning"
            return $null
        }
    }
    catch {
        Write-Log "Error searching for CAB files: $_" -Level "Error"
        return $null
    }
}
...
try {
    ...
    # NEW CODE: Check for existing CAB files first
    $existingCABs = Find-MatchingCAB -KBNumber $KBNumber
    if ($existingCABs) {
		$cabFiles = $existingCABs
		Write-Log "Using existing CAB files" -Level "Info"
    } else {
